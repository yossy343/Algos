...
def AjEd8bitEncode_K_vary(im , hiddenim, img_no):
    imsize = im.shape
    ...
    for i in r_[:imsize[0]:8]:
        for j in r_[:imsize[1]:8]:
            alpha = np.round(abs(newhidden[i][j]*beta2))
            dct_stego[(i+index):(i+8),(j+index):(j+8)] = np.divide(newhidden[i:(i+8-index),j:(j+8-index)],alpha,where=alpha!=0)
            dct_stego[(i+index)][j+index] = newhidden[i][j]/beta1
...
for n in range(0,coverimg): 
    ...
    plt.figure()
    plt.imshow(h_im_o,cmap='gray')
    AjEd8bitEncode_K_vary(im_o , h_im_o,n)
